...
The snippet code above fixes the issue by 1) rejecting bundles exceeding 1000 entries, 2) rejecting individual values exceeding 10KB, 3) preventing memory allocation spikes, and 4) the system remains responsive.
...
if (rule.getConditionComponents() != null &&
rule.getConditionComponents().size() > 50) {
...
The snippet code above fixes the issue by 1) limiting total rules to 100 across the system, 2) limiting each app to 10 rules maximum, 3) validating rule complexity before processing, 4) implementing a timeout to catch infinite rules, and 5) preventing a single app from using all resources.
